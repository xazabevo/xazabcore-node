...
To develop bitcore-node-xazab:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-xazab.git
...
We now will setup symlinks in `bitcore-node-xazab` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-xazab
ln -s ~/bitcoind-rpc-xazab
```
...
cd devnode
mkdir node_modules
touch bitcore-node-xazab.json
touch package.json
```

Edit `bitcore-node-xazab.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-xazab/bin/bitcore-node-xazab start
```