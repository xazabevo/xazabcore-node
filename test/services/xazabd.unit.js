...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/xazab.conf')));
var XazabService = proxyquire('../../lib/services/xazabd', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultXazabConf = fs.readFileSync(path.resolve(__dirname, '../data/default.xazab.conf'), 'utf8');

describe('Xazab Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var xazabd = new XazabService(baseConfig);
      should.exist(xazabd);
    });
    it('will create an instance without `new`', function() {
      var xazabd = XazabService(baseConfig);
      should.exist(xazabd);
    });
    it('will init caches', function() {
      var xazabd = new XazabService(baseConfig);
      should.exist(xazabd.utxosCache);
      should.exist(xazabd.txidsCache);
      should.exist(xazabd.balanceCache);
      should.exist(xazabd.summaryCache);
      should.exist(xazabd.transactionDetailedCache);
      should.exist(xazabd.masternodeListCache);

      should.exist(xazabd.transactionCache);
      should.exist(xazabd.rawTransactionCache);
      should.exist(xazabd.blockCache);
      should.exist(xazabd.rawBlockCache);
      should.exist(xazabd.blockHeaderCache);
      should.exist(xazabd.zmqKnownTransactions);
      should.exist(xazabd.zmqKnownBlocks);
      should.exist(xazabd.lastTip);
      should.exist(xazabd.lastTipTimeout);
    });
    it('will init clients', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.nodes.should.deep.equal([]);
      xazabd.nodesIndex.should.equal(0);
      xazabd.nodes.push({client: sinon.stub()});
      should.exist(xazabd.client);
    });
    it('will set subscriptions', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd._initDefaults({transactionConcurrency: 10});
      xazabd.transactionConcurrency.should.equal(10);
      xazabd._initDefaults({});
      xazabd.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      XazabService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var xazabd = new XazabService(baseConfig);
      var methods = xazabd.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var xazabd = new XazabService(baseConfig);
      var events = xazabd.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('xazabd/rawtransaction');
      events[0].scope.should.equal(xazabd);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('xazabd/transactionlock');
      events[1].scope.should.equal(xazabd);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('xazabd/hashblock');
      events[2].scope.should.equal(xazabd);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('xazabd/addresstxid');
      events[3].scope.should.equal(xazabd);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.subscribe = sinon.stub();
      xazabd.unsubscribe = sinon.stub();
      var events = xazabd.getPublishEvents();

      events[0].subscribe('test');
      xazabd.subscribe.args[0][0].should.equal('rawtransaction');
      xazabd.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      xazabd.unsubscribe.args[0][0].should.equal('rawtransaction');
      xazabd.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      xazabd.subscribe.args[1][0].should.equal('transactionlock');
      xazabd.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      xazabd.unsubscribe.args[1][0].should.equal('transactionlock');
      xazabd.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      xazabd.subscribe.args[2][0].should.equal('hashblock');
      xazabd.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      xazabd.unsubscribe.args[2][0].should.equal('hashblock');
      xazabd.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var xazabd = new XazabService(baseConfig);
      var emitter = {};
      xazabd.subscribe('hashblock', emitter);
      xazabd.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      xazabd.subscribe('rawtransaction', emitter2);
      xazabd.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      xazabd.subscribe('hashblock', emitter1);
      xazabd.subscribe('hashblock', emitter2);
      xazabd.subscribe('hashblock', emitter3);
      xazabd.subscribe('hashblock', emitter4);
      xazabd.subscribe('hashblock', emitter5);
      xazabd.subscriptions.hashblock.length.should.equal(5);

      xazabd.unsubscribe('hashblock', emitter3);
      xazabd.subscriptions.hashblock.length.should.equal(4);
      xazabd.subscriptions.hashblock[0].should.equal(emitter1);
      xazabd.subscriptions.hashblock[1].should.equal(emitter2);
      xazabd.subscriptions.hashblock[2].should.equal(emitter4);
      xazabd.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var xazabd = new XazabService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      xazabd.subscriptions.hashblock= [emitter1];
      xazabd.unsubscribe('hashblock', emitter3);
      xazabd.subscriptions.hashblock.length.should.equal(1);
      xazabd.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      xazabd.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(xazabd.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var xazabd = new XazabService(baseConfig);
      var emitter = new EventEmitter();
      xazabd.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var xazabd = new XazabService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      xazabd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      xazabd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      xazabd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      xazabd.unsubscribeAddress(emitter1);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      xazabd.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      xazabd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      xazabd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      xazabd.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      xazabd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(xazabd, 'unsubscribeAddressAll');
      xazabd.unsubscribeAddress(emitter1);
      xazabd.unsubscribeAddressAll.callCount.should.equal(1);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      xazabd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      xazabd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      xazabd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      xazabd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      xazabd.unsubscribeAddress(emitter1);
      xazabd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      xazabd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      xazabd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(xazabd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var xazabd = new XazabService(baseConfig);
      var config = xazabd._getDefaultConfig();
      config.should.equal(defaultXazabConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a xazab.conf file', function() {
      var TestXazab = proxyquire('../../lib/services/xazabd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var xazabd = new TestXazab(baseConfig);
      xazabd.options.spawn.datadir = '/tmp/.xazab';
      var node = {};
      xazabd._loadSpawnConfiguration(node);
      should.exist(xazabd.spawn.config);
      xazabd.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: xazabcore.Networks.testnet,
          configPath: '/tmp/.xazabcore/xazabcore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var xazabd = new TestXazab(config);
      xazabd.options.spawn.datadir = './data';
      var node = {};
      xazabd._loadSpawnConfiguration(node);
      xazabd.options.spawn.datadir.should.equal('/tmp/.xazabcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestXazab = proxyquire('../../lib/services/xazabd', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badxazab.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var xazabd = new TestXazab(baseConfig);
      (function() {
        xazabd._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(xazabcore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the xazab.conf file', function() {
      var xazabd = new XazabService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      xazabd._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        xazabd.transactionDetailedCache.set(keys[i], {});
        xazabd.utxosCache.set(keys[i], {});
        xazabd.txidsCache.set(keys[i], {});
        xazabd.balanceCache.set(keys[i], {});
        xazabd.summaryCache.set(keys[i], {});
      }
      xazabd._resetCaches();
      should.equal(xazabd.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(xazabd.utxosCache.get(keys[0]), undefined);
      should.equal(xazabd.txidsCache.get(keys[0]), undefined);
      should.equal(xazabd.balanceCache.get(keys[0]), undefined);
      should.equal(xazabd.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      xazabd._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        xazabd.nodes[0].client.getInfo.callCount.should.equal(1);
        xazabd.nodes[1].client.getInfo.callCount.should.equal(1);
        xazabd.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert xazabd-rpc object into JavaScript error', function() {
      var xazabd = new XazabService(baseConfig);
      var error = xazabd._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      xazabd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      xazabd.nodes.push({
        client: {
      ...
      xazabd._initChain(function() {
        log.info.callCount.should.equal(1);
        xazabd.getRawBlock.callCount.should.equal(1);
        xazabd.getRawBlock.args[0][0].should.equal('genesishash');
        xazabd.height.should.equal(5000);
        xazabd.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      xazabd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      xazabd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      xazabd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      xazabcore.Networks.disableRegtest();
      baseConfig.node.network = xazabcore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      ...
      var xazabd = new XazabService(config);
      xazabd._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var xazabd = new XazabService(config);
      xazabd._getDefaultConf().rpcport.should.equal(19998);
    });
    it('will get default rpc port for regtest', function() {
      xazabcore.Networks.enableRegtest();
      var config = {
        node: {
          network: xazabcore.Networks.testnet
        },
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var xazabd = new XazabService(config);
      should.equal(xazabd._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var xazabd = new XazabService(config);
      xazabd._getNetworkConfigPath().should.equal('testnet3/xazab.conf');
    });
    it('will get default rpc port for regtest', function() {
      xazabcore.Networks.enableRegtest();
      var config = {
      ...
      var xazabd = new XazabService(config);
      xazabd._getNetworkConfigPath().should.equal('regtest/xazab.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.node.network = xazabcore.Networks.testnet;
      xazabd._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.node.network = xazabcore.Networks.testnet;
      xazabcore.Networks.enableRegtest();
      xazabd._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd.node.network = xazabcore.Networks.livenet;
      xazabcore.Networks.enableRegtest();
      should.equal(xazabd._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      xazabd._rapidProtectedUpdateTip = sinon.stub();
      xazabd.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      xazabd._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      xazabd._rapidProtectedUpdateTip = sinon.stub();
      xazabd._zmqBlockHandler(node, message);
      xazabd._rapidProtectedUpdateTip.callCount.should.equal(1);
      xazabd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      xazabd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      xazabd._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      xazabd.subscriptions.hashblock.push(emitter);
      emitter.on('xazabd/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        xazabd._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub();
      ...
      xazabd._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      xazabd.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      xazabd.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      xazabd.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub();
      xazabd._resetCaches = sinon.stub();
      xazabd.on('tip', function(height) {
        xazabd._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        xazabd.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      xazabd.on('tip', function() {
        xazabd.syncPercentage.callCount.should.equal(0);
        done();
      });
      xazabd._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using xazabcore.Transaction', function() {
      var xazabd = new XazabService(baseConfig);
      var wif = 'XGLgPK8gbmzU7jcbw34Pj55AXV7SmG6carKuiwtu4WtvTjyTbpwX';
      var privkey = xazabcore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(xazabcore.Networks.testnet);
      var outputAddress = xazabcore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      var tx = xazabcore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: xazabcore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = xazabd._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(xazabcore.Transaction.Output({
        script: xazabcore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = xazabd._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var xazabd = new XazabService(baseConfig);
      var tx = xazabcore.Transaction();
      var address = xazabcore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      tx.addOutput(xazabcore.Transaction.Output({
        script: xazabcore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(xazabcore.Transaction.Output({
        script: xazabcore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      xazabd._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      xazabd.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('xazabd/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      xazabd._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      xazabd.subscriptions.rawtransaction.push(emitter);
      emitter.on('xazabd/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      xazabd._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      xazabd.subscriptions.rawtransaction.push(emitter);
      emitter.on('xazabd/rawtransaction', function() {
        done();
      });
      var node = {};
      xazabd._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      xazabd.subscriptions.transactionlock.push(emitter);
      emitter.on('xazabd/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      xazabd._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var xazabd = new XazabService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      xazabd.subscriptions.transactionlock.push(emitter);
      emitter.on('xazabd/transactionlock', function() {
        done();
      });
      var node = {};
      xazabd._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var xazabd = new XazabService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      xazabd.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._updateTip = sinon.stub();
      xazabd._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      xazabd.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      xazabd._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        xazabd._updateTip.callCount.should.equal(11);
        xazabd._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      xazabd._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        xazabd._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      xazabd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        xazabd._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      xazabd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        xazabd._zmqBlockHandler.callCount.should.equal(0);
        xazabd._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var xazabd = new XazabService(baseConfig);
      var node = {};
      xazabd._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      xazabd._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      xazabd._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      xazabd._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      xazabd.on('tip', function(height) {
        ...
        xazabd.height.should.equal(100);
        done();
      });
      xazabd._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestXazabService = proxyquire('../../lib/services/xazabd', {
        fs: {
          readFile: readFile
        }
      });
      var xazabd = new TestXazabService(baseConfig);
      xazabd.spawnStopTime = 1;
      xazabd._process = {};
      xazabd._process.kill = sinon.stub();
      xazabd._stopSpawnedXazab(function(err) {
        if (err) {
          return done(err);
        }
        xazabd._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      xazabd._process.kill = sinon.stub().throws(error2);
      xazabd._stopSpawnedXazab(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._loadSpawnConfiguration = sinon.stub();
      xazabd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      xazabd._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedXazab', function() {
      var xazabd = new XazabService(baseConfig);
      xazabd._loadSpawnConfiguration = sinon.stub();
      xazabd._stopSpawnedXazab = sinon.stub().callsArgWith(0, new Error('test'));
      xazabd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var xazabd = new TestXazabService(config);
      xazabd.spawn = {};
      xazabd._loadSpawnConfiguration = sinon.stub();
      xazabd._stopSpawnedXazab = sinon.stub().callsArgWith(0, null);
      xazabd.node.stopping = true;
      xazabd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      xazabd._loadSpawnConfiguration = sinon.stub();
      xazabd.spawn = {};
      xazabd.spawn.exec = 'testexec';
      xazabd.spawn.configPath = 'testdir/xazab.conf';
      xazabd.spawn.datadir = 'testdir';
      xazabd.spawn.config = {};
      xazabd.spawn.config.rpcport = 20001;
      xazabd.spawn.config.rpcuser = 'xazab';
      xazabd.spawn.config.rpcpassword = 'password';
      xazabd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      xazabd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      xazabd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      xazabd._initZmqSubSocket = sinon.stub();
      xazabd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      xazabd._checkReindex = sinon.stub().callsArgWith(1, null);
      xazabd._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/xazab.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        xazabd._loadTipFromNode.callCount.should.equal(1);
        xazabd._initZmqSubSocket.callCount.should.equal(1);
        should.exist(xazabd._initZmqSubSocket.args[0][0].client);
        xazabd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        xazabd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(xazabd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn xazabd spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestXazabService = proxyquire('../../lib/services/xazabd', {
        fs: {
      ...
      xazabd._loadSpawnConfiguration = sinon.stub();
      xazabd.spawn = {};
      xazabd.spawn.exec = 'xazabd';
      xazabd.spawn.datadir = '/tmp/xazab';
      xazabd.spawn.configPath = '/tmp/xazab/xazab.conf';
      xazabd.spawn.config = {};
      xazabd.spawnRestartTime = 1;
      xazabd._loadTipFromNode = sinon.stub().callsArg(1);
      xazabd._initZmqSubSocket = sinon.stub();
      xazabd._checkReindex = sinon.stub().callsArg(1);
      xazabd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      xazabd._stopSpawnedXazab = sinon.stub().callsArg(0);
      sinon.spy(xazabd, '_spawnChildProcess');
      xazabd._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            xazabd._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      xazabd._spawnChildProcess(function(err) {
        ...
        xazabd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        xazabd.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn xazabd spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestXazabService = proxyquire('../../lib/services/xazabd', {
        fs: {
      ...
      xazabd._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            xazabd._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      xazabd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      xazabd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      xazabd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      xazabd._spawnChildProcess(function(err) {
        xazabd._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      xazabd._initZmqSubSocket = sinon.stub();
      xazabd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      xazabd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var xazabd = new XazabService(config);
      xazabd.node.stopping = true;
      xazabd.startRetryInterval = 100;
      xazabd._loadTipFromNode = sinon.stub();
      xazabd._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        xazabd._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      xazabd.startRetryInterval = 1;
      var config = {};
      xazabd._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        xazabd._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      xazabd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      xazabd._connectProcess(config, function(err, node) {
        should.not.exist(err);
        xazabd._loadTipFromNode.callCount.should.equal(1);
        xazabd._initZmqSubSocket.callCount.should.equal(1);
        xazabd._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      xazabd.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Xazab configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      xazabd.options = {
        connect: [
          {}
        ]
      ...
      xazabd.start(function(err) {
        xazabd._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var xazabd = new XazabService(baseConfig);
      var node = {};
      xazabd._initChain = sinon.stub().callsArg(0);
      xazabd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      xazabd.options = {
        spawn: {}
      };
      xazabd.start(function(err) {
        should.not.exist(err);
        xazabd.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      xazabd._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      xazabd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      xazabd.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      xazabd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      xazabd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      xazabd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      xazabd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      xazabd.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var xazabd = new XazabService(baseConfig);
      var args = xazabd._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var xazabd = new XazabService(baseConfig);
      var args = xazabd._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      xazabd.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      xazabd.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        xazabd.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      xazabd.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      xazabd.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = xazabd._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = xazabd._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = xazabd._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      xazabd._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        xazabd._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      xazabd.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      xazabd.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      xazabd.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      xazabd.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        xazabd.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var xazabd = new XazabService(baseConfig);
      xazabd.height = 10;
      var confirmations = xazabd._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var xazabd = new XazabService(baseConfig);
      var result = {};
      xazabd._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var xazabd = new XazabService(baseConfig);
      var result = {};
      xazabd._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var xazabd = new XazabService(baseConfig);
      var result = {};
      xazabd._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var xazabd = new XazabService(baseConfig);
      var result = {};
      xazabd._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = xazabd._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var xazabd = new XazabService(baseConfig);
      xazabd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      xazabd.height = 300;
      var addresses = {};
      xazabd._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      xazabd._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var xazabd = new XazabService(baseConfig);
      xazabd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      xazabd._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from xazabcore addresses', function() {
      var addresses = [
        xazabcore.Address('XjxDQFjTNEP9dcrJhBLvy5i1Dobz4x1LJN'),
        xazabcore.Address('7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'),
      ];
      var xazabd = new XazabService(baseConfig);
      var strings = xazabd._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var xazabd = new XazabService(baseConfig);
      (function() {
        xazabd._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      xazabd.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      xazabd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      xazabd.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var xazabd = new XazabService(baseConfig);
      xazabd.maxAddressesQuery = 100;
      xazabd.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      xazabd.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      xazabd.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      xazabd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      xazabd.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      xazabd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      xazabd.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(xazabd, '_paginate');
      xazabd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      xazabd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      xazabd.getAddressSummary(address, options, function(err, summary) {
        xazabd._paginate.callCount.should.equal(1);
        xazabd._paginate.args[0][1].should.equal(0);
        xazabd._paginate.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      xazabd.getAddressSummary(address, options, function(err, summary) {
        ...
        xazabd.getAddressTxids.callCount.should.equal(1);
        xazabd.getAddressBalance.callCount.should.equal(1);
        xazabd.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          xazabd.getAddressTxids.callCount.should.equal(1);
          xazabd.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      xazabd.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginate', function(done) {
      ...
      xazabd._paginate = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      xazabd.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      xazabd.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      xazabd.nodesIndex = 2;
      xazabd.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      xazabd.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      xazabd.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as xazabcore object from height', function(done) {
      var xazabd = new XazabService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      xazabd.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(xazabcore.Block);
        done();
      });
    });
    it('will getblock as xazabcore object', function(done) {
      var xazabd = new XazabService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      xazabd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      xazabd.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      xazabd.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      xazabd.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      xazabd.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      xazabd.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      xazabd.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      xazabd.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#getBlockHeaders', function(){
      ...
      it('will gave error from getBlockHash', function(){
          ...
          xazabd.getBlockHeaders(10, function(err) {
              err.should.be.instanceof(Error);
          });
      });
      it('it will give rpc error from client getblockheaders', function() {
          ...
          xazabd.getBlockHeaders(blockhash, function(err){
              err.should.be.instanceof(Error);
          });
      });
      it("will get an array of block headers", function(){
          ...
          xazabd.getBlockHeaders(_blockHash, function(err, blockHeader){
              should.not.exist(err);
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      xazabd._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      xazabd._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      xazabd._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      xazabd.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      xazabd.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      xazabd.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = xazabcore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      xazabd.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      xazabd.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      xazabd.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = xazabcore.Transaction();
      (function() {
        xazabd.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      xazabd.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      xazabd.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(xazabcore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      xazabd.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      xazabd.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      xazabd.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getBestChainLock', function () {
    it('will give rpc error', function (done) {
      ...
      xazabd.getBestChainLock(function (err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call getBestChainLock and give result', function (done) {
      ...
      xazabd.getBestChainLock(function (err, result) {
        if (err) {
          return done(err);
        }
        should.exist(result);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      xazabd.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.node.getNetworkName = sinon.stub().returns('testnet');
      var getNetworkInfo = sinon.stub().callsArgWith(0, null, {
        result: {
          version: 1,
          protocolversion: 1,
          subversion: '/Xazabd-test:1.0.0/',
          timeoffset: 1,
          connections: 1,
          relayfee: 10,
          warnings: ''
      ...
      xazabd.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
        ...
        should.equal(info.subVersion, '/Xazabd-test:1.0.0/');
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        xazabd.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      var gobject = sinon.stub().callsArgWith(2, null, {
        result: {
          "DataHex": "",
          "DataString": "[[\"proposal\",{\"end_epoch\":1483855193,\"name\":\"7ea9acff5ae83d8c9e217d30a2b4d10ef8f171f8\",\"payment_address\":\"yZ7DYkDHCHfH1dw7rKdYaKcVykZmunbqNL\",\"payment_amount\":84,\"start_epoch\":1483765282,\"type\":1,\"url\":\"https://xazab.xyz\"}]]",
          "Hash": "4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00",
      ...
      xazabd.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
        ...
        should.equal(DataObject.url, 'https://xazab.xyz');
        done();
      });
    });
	...
	describe('#sporksList', function(){
		it('will call client sporks and give result', function(done){
			...
			xazabd.getSpork(function(err, SporkList) {
				if (err) {
					return done(err);
				}
  ...
  describe('#getMNList', function(){
    it('will call client masternode list and give result', function(done){
	    var xazabd = new XazabService(baseConfig);
	    xazabd.isSynced = function(callback) { return callback(null, true) };
	    xazabd.nodes.push({
		    client: {
	    ...
	    xazabd.getMNList(function(err, MNList) {
		    if (err) {
			    return done(err);
		    }
    ...
    it('will return error if one of nodes not synced yet', function(done){
      var xazabd = new XazabService(baseConfig);
      xazabd.isSynced = function(callback) { return callback(null, false) };
      xazabd.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
    ...
    it('will return error if checking synced state of nodes failed', function(done){
      var xazabd = new XazabService(baseConfig);
      xazabd.isSynced = function(callback) { return callback(new Error('Failed')) };
      xazabd.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      xazabd.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      xazabd.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.spawn = {};
      xazabd.spawn.process = new EventEmitter();
      xazabd.spawn.process.kill = sinon.stub();
      xazabd.stop(done);
      xazabd.spawn.process.kill.callCount.should.equal(1);
      xazabd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      xazabd.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      xazabd.spawn.process.kill.callCount.should.equal(1);
      xazabd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      xazabd.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var xazabd = new XazabService(baseConfig);
      xazabd.shutdownTimeout = 300;
      xazabd.spawn = {};
      xazabd.spawn.process = new EventEmitter();
      xazabd.spawn.process.kill = sinon.stub();
      xazabd.stop(function(err) {
