...
describe('Xazab Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.xazabcore/data/xazabd')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/xazab.conf'),
      rpcuser: 'xazab',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/xazab.conf'),
      rpcuser: 'xazab',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/xazab.conf'),
      rpcuser: 'xazab',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 xazabd daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three xazabd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = xazabcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.xazabd.once('tip', function(height) {
