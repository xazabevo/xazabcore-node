...
var xazabcorenode = require('..');
var utils = require('../utils');

function main(servicesPath, additionalServices) {
  /* jshint maxstatements: 100 */

  var version = xazabcorenode.version;
  var create = xazabcorenode.scaffold.create;
  var add = xazabcorenode.scaffold.add;
  var start = xazabcorenode.scaffold.start;
  var remove = xazabcorenode.scaffold.remove;
  var callMethod = xazabcorenode.scaffold.callMethod;
  var findConfig = xazabcorenode.scaffold.findConfig;
  var defaultConfig = xazabcorenode.scaffold.defaultConfig;
  ...
  program
    ...
    .option('-d, --datadir <dir>', 'Specify the Xazab database directory')
  ...
  program
    ...
    .option('-d, --daemon', 'Make xazabcore-node a daemon (running in the background)')
    .action(function(cmd){
      if (cmd.config) {
  ...
  program
    ...
    .action(function(services){
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `xazabcore-node create --help`');
      }
      var opts = {
      ...
      console.log();
      console.log('    $ xazabcore-node add wallet-service');
      console.log('    $ xazabcore-node add insight-api');
      console.log();
    });
  ...
  program
    ...
    .action(function(services){
      ...
      console.log('  Examples:');
      console.log();
      console.log('    $ xazabcore-node remove wallet-service');
      console.log('    $ xazabcore-node remove insight-api');
      console.log();
    });
