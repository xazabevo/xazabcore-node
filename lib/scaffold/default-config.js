...
/**
 * Will return the path and default xazabcore-node configuration. It will search for the
 * configuration file in the "~/.xazabcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.xazabcore');
  var defaultConfigFile = path.resolve(defaultPath, './xazabcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['xazabd', 'web'];
