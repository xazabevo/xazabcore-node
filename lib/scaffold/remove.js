...
/**
 * Will remove a service from xazabcore-node.json
...
/**
 * Will remove the Node.js service and from the xazabcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The xazabcore-node configuration directory
...
function remove(options, done) {
  ...
  var xazabcoreConfigPath = path.resolve(configPath, 'xazabcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(xazabcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a xazabcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to xazabcore-node.json
        removeConfig(xazabcoreConfigPath, service, next);
      });
    }, done
  );
}
