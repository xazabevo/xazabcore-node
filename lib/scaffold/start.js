...
var XazabcoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for xazabd is necessary for this upgrade with the "reindex=1" xazab.conf option. \n' +
                 'There are changes necessary in both xazab.conf and xazabcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/xazabevo/xazabcore-node/blob/xazabd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          xazabd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/xazabd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed xazabcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './xazabcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in xazabcore-node services directory
    ...
    if (servicePackage.xazabcoreNode) {
      serviceModule = service.name + '/' + servicePackage.xazabcoreNode;
    }
 ...
 * @param {Node} node - The Xazabcore Node instance
 */
