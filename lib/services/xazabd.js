...
var Proposal = xazabcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to xazabd in Node.js. Manages starting and
 * stopping xazabd as a child process for application support, as well as connecting
 * to multiple xazabd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Xazab(options) {
  if (!(this instanceof Xazab)) {
    return new Xazab(options);
  }
...
util.inherits(Xazab, Service);

Xazab.dependencies = [];

Xazab.DEFAULT_MAX_TXIDS = 1000;
Xazab.DEFAULT_MAX_HISTORY = 50;
Xazab.DEFAULT_MAX_UTXO = 1000;
Xazab.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Xazab.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Xazab.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Xazab.DEFAULT_SPAWN_RESTART_TIME = 5000;
Xazab.DEFAULT_SPAWN_STOP_TIME = 10000;
Xazab.DEFAULT_TRY_ALL_INTERVAL = 1000;
Xazab.DEFAULT_REINDEX_INTERVAL = 10000;
Xazab.DEFAULT_START_RETRY_INTERVAL = 5000;
Xazab.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Xazab.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Xazab.DEFAULT_INSTANTSEND_FEE = 10000;
Xazab.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Xazab.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Xazab.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Xazab.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Xazab.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Xazab.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Xazab.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Xazab.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Xazab.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Xazab.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Xazab.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Xazab.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Xazab.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Xazab.prototype._initCaches = function() {
...
Xazab.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Xazab.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Xazab.prototype.getPublishEvents = function() {
  return [
    {
...
Xazab.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'xazabd/' + name, 'total:', this.subscriptions[name].length);
};

Xazab.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'xazabd/' + name, 'total:', this.subscriptions[name].length);
};

Xazab.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (xazabcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'xazabd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Xazab.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'xazabd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Xazab.prototype.unsubscribeAddressAll = function(emitter) {
...
Xazab.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Xazab.DEFAULT_CONFIG_SETTINGS;
...
Xazab.prototype._parseXazabConf = function(configPath) {
  var options = {};
...
Xazab.prototype._expandRelativeDatadir = function() {
...
Xazab.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in xazabd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in xazabd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in xazabd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './xazab.conf');

  log.info('Using Xazab config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseXazabConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseXazabConf(networkConfigPath));
  }
...
Xazab.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of xazabcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of xazabcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of xazabcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in xazab.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that xazabd is undergoing a reindex. ' +
             ...
             'of xazabcore-node services will start.');
    node._reindex = true;
  }
};

Xazab.prototype._resetCaches = function() {
 ...
 * @private
 */
Xazab.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Xazab.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Xazab.prototype._initChain = function(callback) {
  var self = this;
...
Xazab.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === xazabcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Xazab.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === xazabcore.Networks.testnet) {
    networkPath = 'testnet3/xazab.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/xazab.conf';
    }
  }
  return networkPath;
};

Xazab.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === xazabcore.Networks.testnet) {
...
Xazab.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('xazabd/hashblock', message.toString('hex'));
    }
  }

};

Xazab.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Xazab.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Xazab Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Xazab.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Xazab.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('xazabd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Xazab.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = xazabcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('xazabd/rawtransaction', message.toString('hex'));
    }

    var tx = xazabcore.Transaction();
    tx.fromString(message);
    var txid = xazabcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Xazab.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('xazabd/transactionlock', message.toString('hex'));
    }

  }
};

Xazab.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Xazab.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Xazab.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Xazab.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Xazab.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Xazab Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Xazab.DEFAULT_REINDEX_INTERVAL);

};

Xazab.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Xazab.prototype._stopSpawnedXazab = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/xazabd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned xazab process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean xazab process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Xazab.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedXazab(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting xazab process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Xazab process unexpectedly exited with code:', code);
        log.warn('Restarting xazab child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Xazab process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn xazabd.'));
      }
...
Xazab.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to xazabd.'));
    }
 ...
 */
Xazab.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Xazab configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Xazab.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Xazab.prototype.syncPercentage = function(callback) {
  var self = this;
...
Xazab.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, xazabcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Xazab.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = xazabcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Xazab.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Xazab.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Xazab.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Xazab.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in xazabcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Xazab.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Xazab.prototype._getConfirmationsDetail = function(transaction) {
...
Xazab.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Xazab.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Xazab.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A xazab transaction id
 * @param {Function} callback
 */
Xazab.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Xazab.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof xazabcore.Address) {
...
Xazab.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Xazab.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Xazab.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Xazab.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Xazabcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Xazab.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = xazabcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Xazab.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Xazab.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Xazab.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Xazab.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Xazab.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Xazabcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Xazab.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Xazab.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Xazab.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Xazab.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Xazab.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Xazab.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Xazab.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Xazab.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Xazab.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Xazab.prototype.govCount = function (callback) {
  var self = this;
...
Xazab.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Xazab.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Xazab.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Xazab.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Xazab.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getBestChainLock = function (callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.getInfo = function(callback) {
  var self = this;
...
Xazab.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Xazab.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('xazabd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('xazabd process did not exit'));
      }
